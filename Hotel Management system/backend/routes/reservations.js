...
async function sendEmail(to, subject, html) {
    try {
        await transporter.sendMail({ from: `"Hotel Nature Love" <${process.env.EMAIL_USER}>`, to, subject, html });
...
function emailWrapper(content) {
    return `<div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;border:1px solid #e0e0e0;border-radius:10px;overflow:hidden">
      <div style="background:#0D1B2A;padding:24px;text-align:center">
        <h1 style="color:#C9A84C;margin:0;font-size:22px">Hotel Nature Love</h1>
      ...
      <div style="background:#f8f9fa;padding:16px;text-align:center;font-size:12px;color:#999;border-top:1px solid #e0e0e0">
        Hotel Nature Love | support@grandazure.com | +60 3-1234 5678
      </div>
    </div>`;
}
...
function scheduleReminders(reservationId, guestName, guestEmail, checkInDate, checkOutDate, roomNumber) {
    ...
    if (checkInDelay > 0) {
        setTimeout(async () => {
            await sendEmail(guestEmail, '⏰ Check-in Reminder — Hotel Nature Love',
                emailWrapper(`
                    <h2 style="color:#0D1B2A">Check-in Reminder</h2>
                    <p>Dear <strong>${guestName}</strong>,</p>
                    <p>Your check-in at <strong>Hotel Nature Love</strong> is in <strong>1 hour</strong>. We are ready to welcome you!</p>
                    ${infoTable([
                        ['Room', roomNumber],
                        ['Check-in Date', checkInDate],
                        ['Check-in Time', '8:00 AM'],
                        ['Location', 'Hotel Nature Love, Main Lobby']
                    ])}
    ...
    if (checkOutDelay > 0) {
        setTimeout(async () => {
            await sendEmail(guestEmail, '⏰ Check-out Reminder — Hotel Nature Love',
                emailWrapper(`
...
router.post('/', authenticate, async (req, res) => {
    try {
        ...
        if (guest_email) {
            ...
            await sendEmail(guest_email, 'Booking Confirmed — Hotel Nature Love',
                emailWrapper(`
                    <h2 style="color:#0D1B2A">Booking Confirmed!</h2>
                    <p>Dear <strong>${guest_name}</strong>,</p>
                    <p>Your reservation at <strong>Hotel Nature Love</strong> is confirmed. We look forward to welcoming you!</p>
                    ${infoTable([
...
router.put('/:id/cancel', authenticate, async (req, res) => {
    try {
        ...
        if (reservation.guest_email) {
            await sendEmail(reservation.guest_email, 'Reservation Cancelled — Hotel Nature Love',
                emailWrapper(`
    ...
    try {
        ...
        if (reservation.guest_email) {
            await sendEmail(reservation.guest_email, 'Welcome — Check-in Confirmed',
                emailWrapper(`
                    <h2 style="color:#0D1B2A">Welcome to Hotel Nature Love!</h2>
    ...
    try {
        ...
        if (isEarly) {
            ...
            await sendEmail(process.env.EMAIL_USER, `Early Checkout — Room ${reservation.room.room_number}`,
                ...
                `));
            if (reservation.guest_email) {
                await sendEmail(reservation.guest_email, 'Early Checkout Confirmed — Hotel Nature Love',
                    emailWrapper(`
...
router.post('/:id/send-reminder', authenticate, authorize('admin', 'receptionist'), async (req, res) => {
    try {
        ...
        if (type === 'checkin') {
            await sendEmail(guest_email, 'Check-in Reminder — Hotel Nature Love',
                emailWrapper(`
                    <h2 style="color:#0D1B2A">Check-in Reminder</h2>
            ...
            await sendEmail(guest_email, 'Check-out Reminder — Hotel Nature Love',
                emailWrapper(`
                    <h2 style="color:#0D1B2A">Check-out Reminder</h2>
